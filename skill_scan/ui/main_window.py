...
from ._palette import (
    ACCENT, ANCHOR, CRITICAL_ACCENT, DEEP_SURFACE, DIVIDER,
    LIGHT_CANVAS, MEDIUM_ACCENT, MUTED_TEXT, SAFE_ACCENT,
)
    ...
    def _build(self):
        ...
        self._back_btn.setStyleSheet(
            self._BTN_BASE.format(
                fg=MUTED_TEXT, hover=ACCENT,
                hover_fg=LIGHT_CANVAS, press=ACCENT,
            )
        )
        ...
        min_btn.setStyleSheet(
            self._BTN_BASE.format(
                fg=MUTED_TEXT, hover=DEEP_SURFACE,
                hover_fg=LIGHT_CANVAS, press="#263347",
            )
        )
        ...
        close_btn.setStyleSheet(
            self._BTN_BASE.format(
                fg=MUTED_TEXT, hover=CRITICAL_ACCENT,
                hover_fg=LIGHT_CANVAS, press="#b91c1c",
            )
        )
...
class _StatusBar(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        self._dot = QLabel("●")
        self._dot.setStyleSheet(f"color:{SAFE_ACCENT};font-size:8px;background:transparent;")
        layout.addWidget(self._dot)

        self._msg = QLabel("Ready")
        self._msg.setStyleSheet(f"color:{MUTED_TEXT};font-size:11px;background:transparent;")
