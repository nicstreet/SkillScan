...
from PyQt6.QtWidgets import (
    ...
    QLabel, QListWidget, QListWidgetItem, QMenu, QPushButton,
...
from ...core.db import (
    ...
    get_or_create_folder, init_db, session,
)
...
from .._palette import (
    ACCENT, ANCHOR, DEEP_SURFACE, DIVIDER,
    HOVER_FOCUS, LIGHT_CANVAS, MEDIUM_ACCENT, MUTED_TEXT, SAFE_ACCENT,
)
...
class _FolderRow(QWidget):
    def __init__(self, path: str, skill_count: int,
                 show_tooltip: bool = True, parent=None):
...
_SEVERITY_RANK = {
    ...
    "clean": 4, "safe": 4, "unknown": 5, None: 6,
}
...
class FoldersView(QWidget):
    ...
    def _make_left_pane(self) -> QWidget:
        ...
        self._folder_list.setContextMenuPolicy(
            Qt.ContextMenuPolicy.CustomContextMenu
        )
        self._folder_list.customContextMenuRequested.connect(
            self._folder_context_menu
        )
    ...
    def _reorder_tiles(self) -> None:
        ...
        ordered = [self._tiles[i.skill_id] for i in sorted_infos if i.skill_id in self._tiles]
    ...
    def _load_folders(self) -> None:
        ...
        try:
            with session() as s:
                folders = (
                    s.query(Folder)
                    .order_by(Folder.added_at)
                    .all()
                )
                folder_data = [
                    (f.id, f.path,
                     s.query(Skill).filter_by(folder_id=f.id).count())
                    for f in folders
                ]
        except Exception:
    ...
    def _on_folder_changed(self, current, _prev) -> None:
        ...
        folder_path = current.data(Qt.ItemDataRole.UserRole + 1)
                    ...
                    if latest:
                        try:
                            ...
                            llm_skipped = bool(
                                ...
                                or any(f.get("rule_id") == "LLM_ANALYSIS_FAILED"
                                       for f in all_finds)
                            )
                            findings = [
    ...
    def _refresh_folder(self, folder_id: int, folder_path: str) -> None:
        self._run_discovery(folder_id, folder_path,
                            lambda _r: self._load_tiles(folder_id))
