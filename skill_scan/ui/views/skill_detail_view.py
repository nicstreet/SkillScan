...
from .._palette import (
    ...
    MUTED_TEXT, SAFE_ACCENT, SOFT_SURFACE,
)
...
_REQUIRED_FIELDS     = ["name", "version", "description", "authors", "license", "tags", "permissions"]
...
class _Sparkline(QWidget):
    ...
    def paintEvent(self, _) -> None:
        ...
        n   = len(self._data)
        seg = min(16, max(6, (self.width() - (n - 1) * 2) // n if n > 1 else self.width()))
        h   = self.height()
        x   = 0
...
class SkillDetailView(QWidget):
    ...
    def _make_header(self) -> QWidget:
        ...
        self._unsafe_badge.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self._results_badge.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self._analyzers_badge.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        ...
        self._sev_badge.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
        self._set_sev_badge(None)
        right.addWidget(self._sev_badge, 0, Qt.AlignmentFlag.AlignRight)

        self._llm_badge = QLabel("LLM  SKIPPED")
        self._llm_badge.setSizePolicy(QSizePolicy.Policy.Fixed, QSizePolicy.Policy.Fixed)
    ...
    def _make_tabs(self) -> QTabWidget:
        ...
        self._compliance_browser.setTextInteractionFlags(Qt.TextInteractionFlag.NoTextInteraction)
    ...
    def _update_info_chips(self, parsed: dict | None, is_safe: bool | None = None) -> None:
        if parsed is None:
            self._unsafe_badge.setVisible(False)
        ...
        safe = parsed.get("is_safe") if parsed.get("is_safe") is not None else bool(is_safe)
    ...
    def _update_compliance(self, path: str, spec_type: str | None = None) -> None:
        ...
        score = max(0, 100
                    - len(missing_req) * _REQ_WEIGHT
                    - len(missing_rec) * _REC_WEIGHT)
        ...
        for f in _REQUIRED_FIELDS:
            ...
            icon   = f'<span style="color:{SAFE_ACCENT};">&#10003;</span>' if present \
                     else f'<span style="color:{CRITICAL_ACCENT};">&#10007;</span>'
            val    = _html.escape(str(meta[f])[:60]) if present else \
                     f'<span style="color:{CRITICAL_ACCENT};">missing</span>'
            parts.append(
        ...
        for f in _RECOMMENDED_FIELDS:
            ...
            val    = _html.escape(str(meta[f])[:60]) if present else \
                     f'<span style="color:{MEDIUM_ACCENT};">not set</span>'
            parts.append(
                f'<tr style="border-bottom:1px solid {DEEP_SURFACE};">'
                f'<td style="padding:5px 8px;width:20px;">{icon}</td>'
                f'<td style="padding:5px 8px;color:{LIGHT_CANVAS};font-weight:600;">'
        ...
        try:
            ...
            with session() as s:
                ...
                if current_hash and current_hash != (skill.file_hash or ""):
                    ...
                    s.commit()
                    _log_activity("Trust auto-revoked — file changed", skill.name or path)
        except Exception:
            return
        self._refresh()
