...
class ScanJob(QObject):
    ...
    def start(self) -> None:
        ...
        # QProcessEnvironment.systemEnvironment() is the default; just inject extras
        ...
        if not self._process.waitForStarted(3000):
            self.error.emit(f"Failed to start skill-scanner: {self._process.errorString()}")
    ...
    def _finalise(self) -> None:
        ...
        remaining_out = bytes(self._process.readAllStandardOutput()).decode("utf-8", errors="replace")
        if remaining_out:
            self._stdout_buf += remaining_out
            for line in remaining_out.splitlines():
                self.output_line.emit(line)

        remaining_err = bytes(self._process.readAllStandardError()).decode("utf-8", errors="replace")
        if remaining_err:
