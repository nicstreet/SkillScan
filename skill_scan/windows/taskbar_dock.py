...
from PyQt6.QtCore import (
    QEasingCurve, QPropertyAnimation, QRect, Qt, pyqtProperty, pyqtSignal,
)
...
class TaskbarDock(QWidget):
    ...
    def _snap(self) -> None:
        ...
        if self._edge == _ABE_BOTTOM:
            self.setGeometry(screen.left(), tb_top - _COLLAPSED_THICKNESS,
                             screen.width(), _COLLAPSED_THICKNESS)
        elif self._edge == _ABE_TOP:
            self.setGeometry(screen.left(), tb_bottom,
                             screen.width(), _COLLAPSED_THICKNESS)
        elif self._edge == _ABE_LEFT:
            self.setGeometry(tb_right, screen.top(),
                             _COLLAPSED_THICKNESS, screen.height())
        else:  # RIGHT
            self.setGeometry(tb_left - _COLLAPSED_THICKNESS, screen.top(),
                             _COLLAPSED_THICKNESS, screen.height())
