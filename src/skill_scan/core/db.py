...
class Folder(Base):
    ...
    skills = relationship(
        "Skill", back_populates="folder", cascade="all, delete-orphan"
    )
    bom_snapshots = relationship(
        "BomSnapshot", back_populates="folder", cascade="all, delete-orphan"
    )
...
class Skill(Base):
    ...
    scan_results = relationship(
        "ScanResult", back_populates="skill", cascade="all, delete-orphan"
    )
    ...
    with session() as s:
        for entry in raw:
            ...
            if isinstance(parsed, dict):
                ...
                severity = (
                    parsed.get("max_severity", "unknown") if not is_safe else "clean"
                )
