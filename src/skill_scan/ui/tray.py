...
class TrayApp:
    def __init__(self, app: QApplication, initial_scan: str | None = None):
        ...
        self._tray = QSystemTrayIcon(
            _make_tray_icon(self._cfg.get("accent_color", "#0ea5e9"))
        )
    ...
    def _build_menu(self):
        ...
        if not has_folders:
            self._toggle_folders.setToolTip(
                "Add folders in Options → Watched Folders first"
            )
    ...
    def _sync_toggle_state(self):
        ...
        self._toggle_clipboard.setChecked(
            self._cfg.get("clipboard_watch_enabled", False)
        )
        self._toggle_folders.setChecked(bool(self._cfg.get("watched_folders")))

    # ------------------------------------------------------------------
    def _notify_delayed(
        self, key: str, checked: bool, get_state, title: str, body: str
    ) -> None:
    ...
    def _on_clipboard_watch_toggled(self, checked: bool):
        ...
        self._notify_delayed(
            ...
            "Clipboard Auto-Scan",
            (
                "Background clipboard scanning enabled."
                if checked
    ...
    def _scan_clipboard(self):
        ...
        if is_temp:
            ...
            self._tray.showMessage(
                "SkillScan",
                "Clipboard text written to temp SKILL.md — scanning…",
            ...
            self._tray.showMessage(
                "SkillScan",
                f"Scanning path from clipboard:\n{path}",
                QSystemTrayIcon.MessageIcon.Information,
                3000,
            )
    ...
    def _on_job_error(self, msg: str, job: ScanJob, dlg):
        if job in self._active_jobs:
            self._active_jobs.remove(job)
        self._tray.showMessage(
            "SkillScan error", msg, QSystemTrayIcon.MessageIcon.Critical, 6000
        )
    ...
    def _quit(self):
        ...
        self._watcher.stop()
        self._clipboard_watcher.configure(
            enabled=False, interval_secs=30, min_chars=200
        )
        self._cleanup_temps()
        self._tray.hide()
        self._app.quit()
