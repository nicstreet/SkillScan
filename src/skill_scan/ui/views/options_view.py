...
class OptionsView(QWidget):
    ...
    def _make_llm_tab(self) -> QWidget:
        ...
        self._llm_provider.setStyleSheet(
            f"QComboBox{{{_field_style}}}" f"QComboBox::drop-down{{border:none;}}"
        )
    ...
    def _make_analyzers_tab(self) -> QWidget:
        ...
        policy_lbl.setStyleSheet(
            f"color:{MUTED_TEXT};font-size:12px;background:transparent;"
        )
