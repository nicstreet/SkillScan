...
class ToggleSwitch(QWidget):
    ...
    def toggle(self) -> None:
        self.setChecked(
            not self._checked
        )  # let setChecked own the state change + animation
    ...
    def paintEvent(self, event):
        ...
        p.drawRoundedRect(
            0, 0, self._TRACK_W, self._TRACK_H, self._TRACK_H / 2, self._TRACK_H / 2
        )
...
class ToggleRow(QWidget):
    ...
    def __init__(
        self, label: str, checked: bool = False, color: str = "#0ea5e9", parent=None
    ):
        super().__init__(parent)
