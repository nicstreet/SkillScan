...
- [ ] **Remediate** action — AI-assisted rewrite of non-compliant sections with diff preview before applying. LLM wiring now exists (`core/llm.py`); own-skill audit view now exists too — unblocked, not yet built.
- [ ] Anthropic-best-practices checks beyond raw spec compliance: trigger clarity, instruction specificity, example coverage, no hallucinated tool names. Partially de-risked by the skill-selection benchmark below (`evals/skill_selection/`) — that harness tests trigger clarity empirically (precision/recall against labeled tasks) rather than via static heuristics.
...
- [x] **Skill-selection benchmark** (validates the trigger-clarity hypothesis empirically, not part of the SkillScan app itself) — `evals/skill_selection/`: 17-skill corpus (15 well-differentiated + 2 deliberately vague controls), 15 hand-labeled tasks, `run_selection_benchmark.py` calls the configured LLM N times per task and computes precision/false-positive rate. Result against `llama3.1:8b` (local): 100% accuracy, 0% false-positive rate across 75 trials (15 tasks × 5 runs). Gap acknowledged: no task's ground truth is itself a vague skill, so this proves the mechanism works and irrelevant skills don't false-trigger, but doesn't yet prove vagueness *causes* a miss — would need a vague/clear twin-pair task to test that directly.
